<x-hops.layout>
    ...
    <div class="relative w-full bg-white py-12 px-4 sm:px-6 lg:px-8 border-b border-hops-light overflow-hidden">
        ...
        <div class="relative max-w-4xl mx-auto text-center">
            <span
                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-hops-light text-hops-mid border border-hops-mid/20 mb-4">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{
            ...
            <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-3xl border border-hops-light shadow-2xs overflow-hidden">
                    <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                        ...
                        <span
                            class="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{{ __('always sums to 100%') }}</span>
                    </div>
        ...
        'feeling' => ['label' => 'Feeling', 'desc' => 'Bitterness & aromaticity'],
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-2">
                                    ...
                                    <span class="text-sm font-black text-hops-mid font-mono tabular-nums"
                                        x-text="pct(moduleWeights.{{ $key }})"></span>
                                </div>
                                <div class="flex items-center gap-3">
                                    <input type="range" :value="moduleWeights.{{ $key }}"
                                        @input="setModuleWeight('{{ $key }}', $event.target.value)"
                                        min="0" max="1" step="0.01"
                                        class="flex-1 h-2 rounded-full accent-hops-accent cursor-pointer" />
                                    <input type="number" :value="Math.round(moduleWeights.{{ $key }} * 100)"
        ...
        'linalool' => 'Linalool (%)',
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-1.5">
                                    ...
                                    <span class="text-sm font-black text-hops-mid font-mono tabular-nums"
                                        x-text="pct(biochemicalWeights.{{ $key }})"></span>
                                </div>
                                <div class="flex items-center gap-3">
                                    <input type="range" :value="biochemicalWeights.{{ $key }}"
        ...
        'feeling' => 'Feeling',
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                                    <div class="h-full bg-hops-accent rounded-full transition-all duration-200"
                                        :style="'width: ' + (moduleWeights.{{ $key }} * 100) + '%'"></div>
                                </div>
                                <span class="text-xs font-bold text-hops-mid font-mono tabular-nums w-10 text-right"
                                    x-text="pct(moduleWeights.{{ $key }})"></span>
                            </div>
                        @endforeach
                    </div>
        ...
        'linalool' => 'Linalool',
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                                    <div class="h-full bg-hops-accent rounded-full transition-all duration-200"
                                        :style="'width: ' + (biochemicalWeights.{{ $key }} * 100) + '%'">
                                    </div>
                                </div>
                                <span class="text-xs font-bold text-hops-mid font-mono tabular-nums w-10 text-right"
                                    x-text="pct(biochemicalWeights.{{ $key }})"></span>
            ...
            <div class="mt-6 bg-white rounded-3xl border border-hops-light shadow-2xs overflow-hidden">
                <div class="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <span
                        class="text-sm text-gray-400 font-bold uppercase tracking-wider">{{ __('Live Preview') }}</span>
                </div>
                <div class="p-6">
