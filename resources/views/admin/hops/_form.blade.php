<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div>
        ...
        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $hop->name ?? '')" required
            autofocus />
    ...
    <div>
        ...
        <x-text-input id="slug" name="slug" type="text" class="mt-1 block w-full font-mono" :value="old('slug', $hop->slug ?? '')"
            required />
    ...
    <div>
        ...
        <x-text-input id="alt_name" name="alt_name" type="text" class="mt-1 block w-full" :value="old('alt_name', $hop->alt_name ?? '')" />
        <x-input-error :messages="$errors->get('alt_name')" class="mt-2" />
    </div>

    <div>
        <x-input-label for="country" :value="__('Country')" />
        <x-text-input id="country" name="country" type="text" class="mt-1 block w-full" :value="old('country', $hop->country ?? '')" />
                ...
                <div class="flex gap-2">
                    <div class="flex-1">
                        ...
                        <x-text-input :id="$field . '_min'" :name="$field . '_min'" type="number" step="0.01"
                            class="mt-1 block w-full text-sm" :value="old($field . '_min', isset($hop) ? $hop->getRawOriginal($field . '_min') : '')" />
                    </div>
                    <div class="flex-1">
                        <x-input-label :for="$field . '_max'" :value="__('Max')" class="text-xs" />
                        <x-text-input :id="$field . '_max'" :name="$field . '_max'" type="number" step="0.01"
                            class="mt-1 block w-full text-sm" :value="old($field . '_max', isset($hop) ? $hop->getRawOriginal($field . '_max') : '')" />
                    </div>
                </div>
...
<div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        @foreach (HopsWeb\Enums\HopDescriptor::cases() as $case)
            @php $field = 'aroma_' . $case->value; @endphp
            <div>
                <x-input-label :for="$field" :value="__($case->name)" />
                <x-text-input :id="$field" :name="$field" type="number" min="0" max="10"
                    class="mt-1 block w-full" :value="old($field, $hop->$field ?? '')" />
...
<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-end ">
        ...
        <div>
            ...
            <x-input-error :messages="$errors->get('aromaticity')" class="mt-2" />
        </div>
        ...
        <div>
            ...
            <x-input-error :messages="$errors->get('thiols')" class="mt-2" />
        </div>
        ...
        <div>
            ...
            <x-input-error :messages="$errors->get('bitterness')" class="mt-2" />
        </div>
        ...
        <div>
            ...
            <x-input-error :messages="$errors->get('maturity')" class="mt-2" />
        </div>
            ...
            <div class="flex gap-2 mt-1">
                <div class="flex-1">
                    ...
                    <x-text-input id="yield_min" name="yield_min" type="number" min="0"
                        class="mt-1 block w-full" :value="old('yield_min', $hop->yield_min ?? '')" />
                ...
                <div class="flex-1">
                    ...
                    <x-text-input id="yield_max" name="yield_max" type="number" min="0"
                        class="mt-1 block w-full" :value="old('yield_max', $hop->yield_max ?? '')" />
